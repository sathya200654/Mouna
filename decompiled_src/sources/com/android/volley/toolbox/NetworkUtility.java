...
final class NetworkUtility {
    ...
    static NetworkResponse getNotModifiedNetworkResponse(Request<?> request, long j, List<Header> list) {
        ...
        if (cacheEntry == null) {
            return new NetworkResponse(UtilsKt.HTTP_RESPONSE_NOT_MODIFIED, (byte[]) null, true, j, list);
        }
        return new NetworkResponse(UtilsKt.HTTP_RESPONSE_NOT_MODIFIED, cacheEntry.data, true, j, HttpHeaderParser.combineHeaders(list, cacheEntry));
    }
    ...
    static byte[] inputStreamToBytes(InputStream inputStream, int i, ByteArrayPool byteArrayPool) throws IOException {
        ...
        try {
            buf = byteArrayPool.getBuf(UserMetadata.MAX_ATTRIBUTE_SIZE);
            while (true) {
                try {
