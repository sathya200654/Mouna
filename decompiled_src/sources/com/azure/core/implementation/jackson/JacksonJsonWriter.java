...
final class JacksonJsonWriter extends JsonWriter {
    private static final ClientLogger LOGGER = new ClientLogger((Class<?>) JacksonJsonWriter.class);
    private JsonWriteContext context = JsonWriteContext.ROOT;
    private final JsonGenerator generator;

    JacksonJsonWriter(JsonGenerator jsonGenerator) {
        this.generator = (JsonGenerator) Objects.requireNonNull(jsonGenerator, "Cannot create a Jackson-based instance of com.azure.json.JsonWriter with a null Jackson JsonGenerator.");
    }
    ...
    public void close() throws IOException {
        if (this.context != JsonWriteContext.COMPLETED) {
            throw LOGGER.logExceptionAsError(new IllegalStateException("Writing of the JSON object must be completed before the writer can be closed. Current writing state is '" + this.context.getWriteState() + "'."));
        }
