...
public class RemoteConfigRegistrar implements ComponentRegistrar {
    ...
    public List<Component<?>> getComponents() {
        ...
        return Arrays.asList(Component.builder(RemoteConfigComponent.class, new Class[]{FirebaseRemoteConfigInterop.class}).name(LIBRARY_NAME).add(Dependency.required(Context.class)).add(Dependency.required(qualified)).add(Dependency.required(FirebaseApp.class)).add(Dependency.required(FirebaseInstallationsApi.class)).add(Dependency.required(AbtComponent.class)).add(Dependency.optionalProvider(AnalyticsConnector.class)).factory(new ComponentFactory() { // from class: com.google.firebase.remoteconfig.RemoteConfigRegistrar$$ExternalSyntheticLambda0
