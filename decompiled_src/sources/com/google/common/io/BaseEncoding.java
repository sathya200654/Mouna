...
public abstract class BaseEncoding {
    ...
    private static class StandardBaseEncoding extends BaseEncoding {
        ...
        public OutputStream encodingStream(final Writer out) {
            ...
            return new OutputStream(this) { // from class: com.google.common.io.BaseEncoding.StandardBaseEncoding.1
                ...
                public void write(int b) throws IOException {
                    this.bitBuffer = (b & Constants.MAX_HOST_LENGTH) | (this.bitBuffer << 8);
        ...
        void encodeChunkTo(Appendable target, byte[] bytes, int off, int len) throws IOException {
            ...
            for (int i2 = 0; i2 < len; i2++) {
                j = (j | ((long) (bytes[off + i2] & UnsignedBytes.MAX_VALUE))) << 8;
            }
                    ...
                    while (true) {
                        ...
                        if (this.this$0.paddingChar != null && this.this$0.paddingChar.charValue() == c) {
                            ...
                            if (i3 >= 8) {
                                ...
                                return (this.bitBuffer >> i4) & Constants.MAX_HOST_LENGTH;
                            }
                        }
                    }
                }
    ...
    private static final class Base16Encoding extends StandardBaseEncoding {
        ...
        void encodeTo(Appendable target, byte[] bytes, int off, int len) throws IOException {
            ...
            Preconditions.checkPositionIndexes(off, off + len, bytes.length);
            for (int i = 0; i < len; i++) {
                int i2 = bytes[off + i] & UnsignedBytes.MAX_VALUE;
    ...
    private static final class Base64Encoding extends StandardBaseEncoding {
        ...
        void encodeTo(Appendable target, byte[] bytes, int off, int len) throws IOException {
            ...
            while (len >= 3) {
                int i2 = off + 2;
                int i3 = ((bytes[off + 1] & UnsignedBytes.MAX_VALUE) << 8) | ((bytes[off] & UnsignedBytes.MAX_VALUE) << 16);
                off += 3;
                int i4 = i3 | (bytes[i2] & UnsignedBytes.MAX_VALUE);
        ...
        int decodeTo(byte[] target, CharSequence chars) throws DecodingException {
            ...
            while (i < charSequenceTrimTrailingPadding.length()) {
                ...
                if (i3 < charSequenceTrimTrailingPadding.length()) {
                    ...
                    int i6 = i2 + 2;
                    target[i4] = (byte) ((iDecode2 >>> 8) & Constants.MAX_HOST_LENGTH);
                    if (i5 < charSequenceTrimTrailingPadding.length()) {
                        i += 4;
                        i2 += 3;
                        target[i6] = (byte) ((iDecode2 | this.alphabet.decode(charSequenceTrimTrailingPadding.charAt(i5))) & Constants.MAX_HOST_LENGTH);
                    } else {
                        i2 = i6;
                        i = i5;
                    }
