...
public final class InternetDomainName {
    ...
    InternetDomainName(String name) {
        String lowerCase = Ascii.toLowerCase(DOTS_MATCHER.replaceFrom(name, '.'));
    ...
    private int findSuffixOfType(Optional<PublicSuffixType> desiredType) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (i > 0 && matchesType(desiredType, Optional.fromNullable((PublicSuffixType) PublicSuffixPatterns.UNDER.get(strJoin)))) {
                return i - 1;
            }
            if (matchesType(desiredType, Optional.fromNullable((PublicSuffixType) PublicSuffixPatterns.EXACT.get(strJoin)))) {
                return i;
            }
    ...
    private InternetDomainName ancestor(int levels) {
        ...
        ImmutableList immutableListSubList = immutableList.subList(levels, immutableList.size());
        int length = levels;
        for (int i = 0; i < levels; i++) {
            length += ((String) this.parts.get(i)).length();
        }
