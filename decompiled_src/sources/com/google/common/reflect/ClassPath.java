...
public final class ClassPath {
    ...
    public static ClassPath from(ClassLoader classloader) throws IOException {
        ...
        while (it2.hasNext()) {
            builder.addAll(((LocationInfo) it2.next()).scanResources(hashSet));
        }
            ...
            try {
                ...
                try {
                    ...
                    while (it.hasNext()) {
                        ...
                        if (scannedUris.add(file2.getCanonicalFile())) {
                            scan(file2, scannedUris, builder);
                        }
                    }
    ...
    static ImmutableMap<File, ClassLoader> getClassPathEntries(ClassLoader classloader) {
        ...
        while (it.hasNext()) {
            URL url = (URL) it.next();
            if (url.getProtocol().equals(UtilsKt.SCHEME_FILE)) {
        ...
        return ImmutableMap.copyOf(linkedHashMapNewLinkedHashMap);
    }
        ...
        for (String str : Splitter.on(StandardSystemProperty.PATH_SEPARATOR.value()).split(StandardSystemProperty.JAVA_CLASS_PATH.value())) {
            try {
                try {
                    ...
                    builder.add(new URL(UtilsKt.SCHEME_FILE, (String) null, new File(str).getAbsolutePath()));
                }
    ...
    static String getClassName(String filename) {
        return filename.substring(0, filename.length() - CLASS_FILE_NAME_EXTENSION.length()).replace(JsonPointer.SEPARATOR, '.');
    }
