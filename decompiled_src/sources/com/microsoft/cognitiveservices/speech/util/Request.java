...
class Request implements Runnable {
    ...
    Request(HttpClient httpClient, String str, String str2, byte[] bArr, String str3, int[] iArr, byte[] bArr2) throws IOException {
        ...
        HttpURLConnection httpURLConnection = (HttpURLConnection) ((URLConnection) FirebasePerfUrlConnection.instrument(httpClient.newUrl(str).openConnection()));
        ...
        try {
            try {
                ...
                try {
                    ...
                    try {
                        BufferedInputStream bufferedInputStream = responseCode >= 300 ? new BufferedInputStream(this.m_connection.getErrorStream()) : new BufferedInputStream(this.m_connection.getInputStream());
                        byte[] bArr2 = new byte[UserMetadata.MAX_ATTRIBUTE_SIZE];
                        Vector<byte[]> vector2 = new Vector();
                        int i2 = 0;
                        while (true) {
                            int i3 = bufferedInputStream.read(bArr2, 0, UserMetadata.MAX_ATTRIBUTE_SIZE);
                            if (i3 < 0) {
                                break;
                            } else if (i3 > 0) {
