...
public final class SavedStateRegistryImpl {
    ...
    public final Bundle consumeRestoredStateForKey(String key) {
        ...
        Bundle bundleM33constructorimpl = SavedStateReader.m33constructorimpl(bundle);
        Bundle bundleM90getSavedStateimpl = SavedStateReader.m34containsimpl(bundleM33constructorimpl, key) ? SavedStateReader.m90getSavedStateimpl(bundleM33constructorimpl, key) : null;
        SavedStateWriter.m155removeimpl(SavedStateWriter.m119constructorimpl(bundle), key);
        if (SavedStateReader.m111isEmptyimpl(SavedStateReader.m33constructorimpl(bundle))) {
            this.restoredState = null;
        }
        return bundleM90getSavedStateimpl;
    }
    ...
    public final void performRestore$savedstate_release(Bundle savedState) throws KotlinNothingValueException {
        if (!this.attached) {
            performAttach();
        }
        ...
        Bundle bundleM90getSavedStateimpl = null;
        if (savedState != null) {
            Bundle bundleM33constructorimpl = SavedStateReader.m33constructorimpl(savedState);
            if (SavedStateReader.m34containsimpl(bundleM33constructorimpl, SAVED_COMPONENTS_KEY)) {
                bundleM90getSavedStateimpl = SavedStateReader.m90getSavedStateimpl(bundleM33constructorimpl, SAVED_COMPONENTS_KEY);
            }
        }
        this.restoredState = bundleM90getSavedStateimpl;
    ...
    public final void performSave$savedstate_release(Bundle outBundle) {
        ...
        Bundle bundleM119constructorimpl = SavedStateWriter.m119constructorimpl(bundleBundleOf);
        Bundle bundle = this.restoredState;
        if (bundle != null) {
            SavedStateWriter.m123putAllimpl(bundleM119constructorimpl, bundle);
        }
        synchronized (this.lock) {
            for (Map.Entry entry2 : this.keyToProviders.entrySet()) {
                SavedStateWriter.m146putSavedStateimpl(bundleM119constructorimpl, (String) entry2.getKey(), ((SavedStateRegistry.SavedStateProvider) entry2.getValue()).saveState());
            }
            Unit unit = Unit.INSTANCE;
        }
        if (SavedStateReader.m111isEmptyimpl(SavedStateReader.m33constructorimpl(bundleBundleOf))) {
            return;
        }
        SavedStateWriter.m146putSavedStateimpl(SavedStateWriter.m119constructorimpl(outBundle), SAVED_COMPONENTS_KEY, bundleBundleOf);
    }
}
