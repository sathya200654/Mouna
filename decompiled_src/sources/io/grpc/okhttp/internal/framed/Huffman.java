...
class Huffman {
    ...
    void encode(byte[] bArr, OutputStream outputStream) throws IOException {
        ...
        if (i > 0) {
            outputStream.write((int) ((j << (8 - i)) | ((long) (Constants.MAX_HOST_LENGTH >>> i))));
        }
    }
    ...
    byte[] decode(byte[] bArr) throws IOException {
        ...
        for (byte b : bArr) {
            ...
            i2 += 8;
            while (i2 >= 8) {
                node = node.children[(i >>> (i2 - 8)) & Constants.MAX_HOST_LENGTH];
        ...
        while (i2 > 0) {
            Node node2 = node.children[(i << (8 - i2)) & Constants.MAX_HOST_LENGTH];
